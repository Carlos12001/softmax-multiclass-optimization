...
classdef optimizer < handle
  ...
  properties (Access = private)
    ...
    s = []; ## Last squared gradient average
  ...
  methods (Access = private)
    ...
    ## theta update with rmsprop
    function theta_new = updateRMSprop(self,theta_current,gradient)
      self.s = self.beta2*self.s + (1-self.beta2)*gradient.^2;
      theta_new = theta_current - ...
                  self.alpha*gradient./ (sqrt(self.s) + self.epsilon);
    endfunction


    ## theta update with adam
    function theta_new = updateAdam(self,theta_current,gradient)
      self.v = self.beta1*self.v + (1-self.beta1)*gradient;
      self.s = self.beta2*self.s + (1-self.beta2)*gradient.^2;
      theta_new = theta_current - ...
                  self.alpha*self.v./ (sqrt(self.s) + self.epsilon);
    endfunction
  ...
  methods (Access = public)
    ...
    function [pos,errors]=minimize(self,J,theta0,Xo,Yo)
      ...
      grad = full(reshape(loss.diff(theta), size(theta'))'); # Compute the gradient with auto-differentation
      self.v = full(grad);  # Gradient at current position respect to theta

      ## Initialize the squared gradient average
      self.s = zeros(rows(theta0),columns(theta0));
